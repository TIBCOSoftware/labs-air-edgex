...
func (d *Driver) HandleReadCommands(deviceName string, protocols map[string]models.ProtocolProperties, reqs []sdkModel.CommandRequest) ([]*sdkModel.CommandValue, error) {
	...
	for i, req := range reqs {
		...
		if req.DeviceResourceName == "Status" {
			...
			cv, _ = sdkModel.NewFloat32Value("Voltage", resTime, float32(floatVal))
			responses[i] = cv
			...
			floatVal, _ := strconv.ParseFloat(val, 32)
			cv, _ = sdkModel.NewFloat32Value("Current", resTime, float32(floatVal))
			responses[i] = cv
			...
			cv, _ = sdkModel.NewInt16Value("Energy", resTime, int16(intVal))
			responses[i] = cv
		}
