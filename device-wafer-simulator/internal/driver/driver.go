...
func (d *Driver) Initialize(lc logger.LoggingClient, asyncCh chan<- *sdkModel.AsyncValues, deviceCh chan<- []sdkModel.DiscoveredDevice) error {
	lc.Info(fmt.Sprintf("Initialize Device: %s", "WaferDevices"))
	d.Logger = lc
	d.AsyncCh = asyncCh
...
func publishSimulatedData() {
	...
	for deviceName := range dynamicSimDevices {
		...
		val = b64.StdEncoding.EncodeToString([]byte(GetWaferData(ind)))
		cv = sdkModel.NewStringValue("wafer_snapshot", currentts, val)
		commandValues[0] = cv
