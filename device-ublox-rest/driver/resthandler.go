...
const (
	apiDevicePath  = common.ApiBase + "/device/"
	apiDeviceRoute  = common.ApiBase + "/device/{" + common.DeviceName + "}"
...
type UbloxSensor struct {
	Temp float32 `json:"temp"`
}
type UbloxGnss struct {
	Lat float32 `json:"lat"`
	Lon float32 `json:"lon"`
	Qty int16 `json:"qty"`
}
type UbloxLocation struct {
	Gnss UbloxGnss `json:"gnss"`
}
type UBloxMsg struct {
	Event UbloxEvent
	Sensor UbloxSensor
	Loc UbloxLocation
}
...
func (handler RestHandler) Start() error {
	...
	if err := handler.service.AddRoute(apiDeviceRoute, handler.addContext(deviceHandler), http.MethodPost); err != nil {
		return fmt.Errorf("unable to add required route: %s: %s", apiDeviceRoute, err.Error())
	}

	handler.logger.Infof("Route %s added.", apiDeviceRoute)
...
func (handler RestHandler) processAsyncRequest(writer http.ResponseWriter, request *http.Request) {
	...
	handler.logger.Debugf("Received POST request URI%s", request.RequestURI)

	if strings.HasPrefix(string(request.RequestURI), apiDevicePath) {
		handler.processDeviceRequest(writer, request)
	} else {
		handler.processResourceRequest(writer, request)
	}
}

func (handler RestHandler) processDeviceRequest(writer http.ResponseWriter, request *http.Request) {
	vars := mux.Vars(request)
	deviceName := vars[common.DeviceName]

	handler.logger.Debugf("Received POST for Device=%s", deviceName)
	...
	handler.logger.Debugf("Received contentType=%s", contentType)

	// var reading map[string]interface{}
	var reading UBloxMsg
	...
	handler.logger.Debugf("Received body=%v", body)

	json.Unmarshal(body, &reading)

	handler.logger.Debugf("Received reading=%v", reading)

	handler.logger.Debugf("Received reading Lat=%d", reading.Loc.Gnss.Lat)

	// Handle temperature resource reading
	...
	value, err := handler.newCommandValue(deviceResource, reading.Sensor.Temp, deviceResource.Properties.ValueType, contentType)
	if err != nil {
	...
	// Handle location resource reading
	...
	locationReading := fmt.Sprintf("%f|%f|%d", reading.Loc.Gnss.Lat, reading.Loc.Gnss.Lon, reading.Loc.Gnss.Qty)
	value, err = handler.newCommandValue(deviceResource, locationReading, deviceResource.Properties.ValueType, contentType)
	if err != nil {
		handler.logger.Errorf("Incoming reading ignored. Unable to create Command Value for Device=%s Command=%s: %s",
			deviceName, resourceName, err.Error())
		http.Error(writer, err.Error(), http.StatusBadRequest)
...
func (handler RestHandler) processResourceRequest(writer http.ResponseWriter, request *http.Request) {
	vars := mux.Vars(request)
	deviceName := vars[common.DeviceName]
