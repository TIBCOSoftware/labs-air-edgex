...
// MQTTSendMetadata will send data from the previous function to the specified Endpoint via MQTT.
// If no previous function exists, then the event that triggered the pipeline will be used.
// An empty string for the mimetype will default to application/json.
func (sender MQTTSender) MQTTSendMetadata(msg string) (bool, interface{}) {

	loggingClient.Trace(fmt.Sprintf("Sending Message: [%s]\n", msg))
