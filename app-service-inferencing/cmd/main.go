...
import (
	...
	inttransforms "github.com/TIBCOSoftware/labs-air/edgexfoundry/app-service-inferencing/internal/transforms"
...
const (
	serviceKey = "app-service-inferencing"
)
		...
		if inttransforms.IsInferable(event.Readings[0].Device, event.Readings[0].Name) {
			...
			jsondat := &msgStruct{
				...
				Readings: []models.Reading{
					{
						Id:          event.Readings[0].Id,
						Origin:      event.Readings[0].Origin,
						Device:      event.Readings[0].Device,
						Name:        event.Readings[0].Name,
						Value:       event.Readings[0].Value,
						BinaryValue: event.Readings[0].BinaryValue,
						ValueType:   event.Readings[0].ValueType,
					},
					{
						Id:          uuid.New().String(),
						Origin:      event.Readings[0].Origin,
						Device:      event.Readings[0].Device,
						Name:        event.Readings[0].Name + "_Inferred",
						Value:       prediction,
						BinaryValue: nil,
						ValueType:   "String",
					},
			...
			// Post inference result back to the bus
			...
			// Put original message on the bus
			payload, _ := json.Marshal(event)
			// By calling Complete, the original events will be posted back to the message bus on the new topic defined in the configuration.
