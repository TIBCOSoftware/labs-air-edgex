...
import (
	...
	inttransforms "github.com/TIBCOSoftware/labs-air/edgexfoundry/app-service-filtering/internal/transforms"
...
const (
	serviceKey = "app-service-filtering"
)
...
func main() {
	...
	inttransforms.InitializeFiltering()

	// Add HTTP Route for ModelConf Dynamic Configuration
	edgexSdk.AddRoute("/api/v1/setFiltersConf", processSetFilters, "POST")
...
func processEvent(edgexcontext *appcontext.Context, params ...interface{}) (bool, interface{}) {
	...
	if event, ok := params[0].(models.Event); ok {
		...
		// Check to see if resource needs to be filtered
		if !inttransforms.IsFiltered(event.Readings[0].Device, event.Readings[0].Name) {
...
func processSetFilters(writer http.ResponseWriter, req *http.Request) {

	body, err := ioutil.ReadAll(req.Body)

	if err != nil {
		fmt.Printf("Processing setFilters request ERROR\n")
	}

	inttransforms.RegisterFilters(body)
