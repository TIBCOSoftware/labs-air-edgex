...
import (
	...
	"github.com/TIBCOSoftware/labs-air/edgexfoundry/device-camera-onvif/internal/pkg/axis"
	"github.com/TIBCOSoftware/labs-air/edgexfoundry/device-camera-onvif/internal/pkg/bosch"
	"github.com/TIBCOSoftware/labs-air/edgexfoundry/device-camera-onvif/internal/pkg/client"
	"github.com/TIBCOSoftware/labs-air/edgexfoundry/device-camera-onvif/internal/pkg/noop"
)
...
func (d *Driver) HandleReadCommands(deviceName string, protocols map[string]contract.ProtocolProperties, reqs []sdkModel.CommandRequest) ([]*sdkModel.CommandValue, error) {
	...
	d.lc.Info(fmt.Sprintf("Handling read command for:  %s", deviceName))
	...
	for i, req := range reqs {
		...
		case "onvif_device_information":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_device_information"))
		...
		case "onvif_profile_information":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_profile_information"))
			...
			responses[i] = cv
		case "OnvifDateTime":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "OnvifDateTime"))
			...
			responses[i] = cv
		case "OnvifHostname":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "OnvifHostname"))
			...
			responses[i] = cv
		case "onvif_dns":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_dns"))
		...
		case "onvif_network_interfaces":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_network_interfaces"))
		...
		case "onvif_network_protocols":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_network_protocols"))
		...
		case "onvif_network_default_gateway":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_network_default_gateway"))
			...
			responses[i] = cv
		case "onvif_ntp":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_ntp"))
		...
		case "onvif_system_reboot":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_system_reboot"))
			...
			responses[i] = cv
		case "onvif_users":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_users"))
			...
			responses[i] = cv
		case "onvif_snapshot":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "onvif_snapshot"))
			data, err := onvifClient.GetSnapshot()

			saveImageToFile(data)

			d.lc.Info(fmt.Sprintf("Saved image to file:  %s", "onvif_snapshot"))
			...
			responses[i] = cv
		case "OnvifStreamURI":
			d.lc.Info(fmt.Sprintf("Handling:  %s", "OnvifStreamURI"))
...
func saveImageToFile(imgByte []byte) {
	...
	out, _ := os.Create("./onvifsnapshot.jpeg")
	defer out.Close()
