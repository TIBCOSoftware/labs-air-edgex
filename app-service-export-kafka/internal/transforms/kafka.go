...
// KafkaSendMetadata will send data from the previous function to the specified Endpoint via Kafka.
// If no previous function exists, then the event that triggered the pipeline will be used.
// An empty string for the mimetype will default to application/json.
func (sender KafkaSender) KafkaSendMetadata(msg string) (bool, interface{}) {

	loggingClient.Info(fmt.Sprintf("KafkaSend metadata: "))
